...
namespace try_bi
{
    public partial class uc_coba : UserControl
    {
            ...
            {                
                ...
                if (ckon.sqlDataRd.HasRows)
                {
                    ...
                    {                        
                        ...
                        omniStore = ckon.sqlDataRd["OMNISTORECODE"].ToString();
                        ...
                        dgv_purchase.Rows[dgRows].Cells[4].Value = omniStore;
                        dgv_purchase.Rows[dgRows].Cells[5].Value = qty;
                        dgv_purchase.Rows[dgRows].Cells[6].Value = PRICE_GLOBAL;
                        dgv_purchase.Rows[dgRows].Cells[7].Value = disc;
                        dgv_purchase.Rows[dgRows].Cells[8].Value = discAmount;
                        dgv_purchase.Rows[dgRows].Cells[9].Value = sub_total == 0 ? "0,00" : sub_total.ToString();                       
        ...
        private void dgv_purchase_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            {
                string artId = dgv_purchase.Rows[e.RowIndex].Cells["articleId"].Value.ToString();
                string artName = dgv_purchase.Rows[e.RowIndex].Cells["ItemName"].Value.ToString();
                string price = dgv_purchase.Rows[e.RowIndex].Cells["ArtPrice"].Value.ToString();
                int qty = Convert.ToInt32(dgv_purchase.Rows[e.RowIndex].Cells["Quantity"].Value);
            ...
            else if (dgv_purchase.Columns[e.ColumnIndex].Name == "Delivery")
            {
                string artId = dgv_purchase.Rows[e.RowIndex].Cells["articleId"].Value.ToString();
                string artName = dgv_purchase.Rows[e.RowIndex].Cells["ItemName"].Value.ToString();                
                string qty = dgv_purchase.Rows[e.RowIndex].Cells["Quantity"].Value.ToString();
                string omniStore = dgv_purchase.Rows[e.RowIndex].Cells["OmniStore"].Value.ToString();

                W_DeliveryAddress deliveryAddress = new W_DeliveryAddress(f1);
                deliveryAddress.get_data(artId, artName, qty, l_transaksi.Text, id_spg, omniStore, store_code, false);
                deliveryAddress.ShowDialog();
            }
        }
        ...
        private void dgv_purchase_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (dgv_purchase.Columns[e.ColumnIndex].Name == "Delete")
            {
                ...
                String cmd_delete = "DELETE FROM [tmp].[" + store_code + "] WHERE TRANSACTION_ID='" + l_transaksi.Text + "' AND ARTICLE_ID='" + id + "' AND OMNISTORECODE = '"+ omniStore +"'";
                ...
                String del_promo_list = "DELETE FROM promotion";
                sql.ExecuteNonQuery(del_promo_list);
            }

            if (dgv_purchase.Columns[e.ColumnIndex].Name == "ArtPrice")
            {                         
                try
                {
        ...
        public void cek_article()
        {
            ...
            try
            {
                ...
                cmd_article = "SELECT * FROM article WHERE article.ARTICLE_ID_ALIAS = '" + t_barcode.Text + "'";
                ...
                if (hasRows)
                {
                    ...
                    if (ckon.sqlDataRdHeader.HasRows)
                    {
                        while (ckon.sqlDataRdHeader.Read())
                        {
                            ...
                            price = Convert.ToInt32(ckon.sqlDataRdHeader["PRICE"].ToString());                            
                        }                        
                ...
                else
                {
                    ...
                    cmd_article = "SELECT * FROM article WHERE ARTICLE_ID = '" + upper + "'";
                    ckon.sqlDataRdHeader = sql.ExecuteDataReader(cmd_article, ckon.sqlCon());
        ...
        public void save_trans_header()
        {
            ...
            try
            {
                ...
                {
                    String cmd_insert = "INSERT INTO [transaction] (TRANSACTION_ID,STORE_CODE,CUSTOMER_ID,EMPLOYEE_ID,SPG_ID,STATUS, DATE, TIME, CUST_ID_STORE, CURRENCY,ID_SHIFT,ID_C_STORE,COMP_NAME,TRANSACTION_TYPE) VALUES ('" + l_transaksi.Text + "','" + store_code + "' ,'" + t_custId.Text + "','" + id_employe2 + "' ,'" + sub_string2 + "','0','" + DateHeaderTrans + "', '" + mydate.ToLocalTime().ToString("H:mm:ss") + "', '" + cust_id_store2 + "', '" + nm_cur2 + "','" + id_shift + "','" + id_CStore + "','" + comp_name + "','1') ";
        ...
        public void insertIntoTransactionTable()
        {
            CRUD sql = new CRUD();
            koneksi ckonCheck = new koneksi();
            int price, qty_Trans, subtotal_Trans, disc_Trans, discAmount, maxDiscQty, maxDiscAmount, isService, isOmniTrans, omniShippingCost;
            String spg_Id, article_Id, discountCode, discountType, discountDesc, omniStoreCode, omniDeliveryAddress, omniCourier;            
            ...
            if (ckon.sqlDataRd.HasRows)
            {
                while (ckon.sqlDataRd.Read())
                {
                    ...
                    isOmniTrans = Convert.ToInt32(ckon.sqlDataRd["IS_OMNITRANS"]);
                    omniStoreCode = Convert.ToString(ckon.sqlDataRd["OMNISTORECODE"]);
                    omniDeliveryAddress = Convert.ToString(ckon.sqlDataRd["DELIVERYCUSTADDRESS"]);
                    omniShippingCost = Convert.ToInt32(ckon.sqlDataRd["OMNISHIPPINGCOST"]);
                    omniCourier = Convert.ToString(ckon.sqlDataRd["OMNICOURIER"]);


                    String cmd_check = "SELECT * FROM transaction_line WHERE TRANSACTION_ID ='" + l_transaksi.Text + "' AND ARTICLE_ID='" + article_Id + "' AND OMNISTORECODE = '"+ omniStoreCode + "'";
                    ...
                    if (ckonCheck.sqlDataRd.HasRows)
                    {
                        String cmd_update = "UPDATE transaction_line SET QUANTITY='" + qty_Trans + "',DISCOUNT='" + disc_Trans + "' ,SUBTOTAL='" + subtotal_Trans + "', PRICE = '" + price + "', DISCOUNTAMOUNT = '" + discAmount + "', MAXDISCOUNTQUANTITY = '" + maxDiscQty + "', MAXDISCOUNTAMOUNT = '" + maxDiscAmount + "', DISCOUNTSETUPLINES = 0 " +
                            "WHERE TRANSACTION_ID='" + l_transaksi.Text + "' AND ARTICLE_ID='" + article_Id + "' AND OMNISTORECODE = '" + omniStoreCode + "'";
                    ...
                    else
                    {
                        ...
                        string cmd_insert = "INSERT INTO transaction_line (TRANSACTION_ID, ARTICLE_ID, QUANTITY, PRICE, DISCOUNT, SUBTOTAL, SPG_ID, DISCOUNT_CODE, DISCOUNT_TYPE, DISCOUNT_DESC, IS_SERVICE, IS_OMNITRANS, OMNISTORECODE, DELIVERYCUSTADDRESS, OMNISHIPPINGCOST, OMNICOURIER, DISCOUNTAMOUNT, MAXDISCOUNTQUANTITY, MAXDISCOUNTAMOUNT, DISCOUNTSETUPLINES, DELIVERYTYPE) " +
                                            "VALUES ('" + l_transaksi.Text + "','" + article_Id + "', '" + qty_Trans + "', '" + price + "', '" + disc_Trans + "' ,'" + subtotal_Trans + "', '" + spg_Id + "','" + discountCode + "','" + discountType + "','" + discountDesc + "', '" + isService + "', '" + isOmniTrans + "', '" + omniStoreCode + "', " +
                                            "'" + omniDeliveryAddress + "', '" + omniShippingCost + "', '" + omniCourier + "', '" + discAmount + "', '" + maxDiscQty + "', '" + maxDiscAmount + "', 0, '')";
                        sql.ExecuteNonQuery(cmd_insert);
                    }
                }
            }
