...
namespace try_bi
{
    public partial class UC_MO_List : UserControl
    {
        ...
        public void retreive(string sort = "", string filter = "", string search = "")
        {
            ...
            {
                ckon.sqlCon().Open();
                String cmd = "SELECT  mutasiorder_line.ARTICLE_ID ,mutasiorder_line.QUANTITY, mutasiorder_line.UNIT, article.ARTICLE_NAME, article.SIZE_ID, article.COLOR_ID, article.PRICE FROM mutasiorder_line, article  WHERE article.ARTICLE_ID = mutasiorder_line. ARTICLE_ID AND mutasiorder_line.MUTASI_ORDER_ID='" + l_transaksi.Text + "' ";
                if (filter != "")
                {
