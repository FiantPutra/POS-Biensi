...
namespace try_bi
{
    public partial class w_split : Form
    {
        ...
        public void hitung()
        {
            ...
            if (change_double == 0)
            {
                String cmd_update = "UPDATE transaction SET DISCOUNT='"+ field_diskon2 +"',TOTAL='" + total_double + "', STATUS='1', PAYMENT_TYPE='2', CASH='"+ double_Tcash +"', EDC='"+ double_Tedc +"', CHANGEE='"+ change_double +"', BANK_NAME='"+ id_bank+"', NO_REF='"+ t_rev.Text+"' WHERE TRANSACTION_ID='"+ id_trans +"'";
            ...
            else
            {
                ...
                String cmd_update = "UPDATE transaction SET TOTAL='" + total_double + "', STATUS='1', PAYMENT_TYPE='2', CASH='" + double_Tcash + "', EDC='" + double_Tedc + "', CHANGEE='" + change_double + "', BANK_NAME='" + id_bank + "', NO_REF='" + t_rev.Text + "' WHERE TRANSACTION_ID='" + id_trans + "'";
                edit.ExecuteNonQuery(cmd_update);
                uc_kembalian uc_k = new uc_kembalian(f1);
                f1.p_kanan.Controls.Clear();
                if (!f1.p_kanan.Controls.Contains(uc_kembalian.Instance))
