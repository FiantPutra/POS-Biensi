...
namespace try_bi
{
    class PrintThermal
    {
        ...
        String nm_store, trans_id, art_id, size, pay_type, diskon_format, price_format, SubTotal_format, mata_uang, sign, SubTotal_format2, bank_id, noref, bank_name, bank_name2, bank_id2, noref2, date, time, discount_desc, disc_desc_real;
        ...
        public void get_currency()
        {
            ...
            try
            {
                ...
                if (ckon.sqlDataRd.HasRows)
                {
                    ...
                    {
                        sign = ckon.sqlDataRd["NAME"].ToString();
                    }
                }
            }
            catch (Exception e)
        ...
        public void get_nm_store()
        {
            ...
            try
            {
                ...
                if (ckon.sqlDataRd.HasRows)
                {
                    while (ckon.sqlDataRd.Read())
                    {
                        nm_store = ckon.sqlDataRd["NAME"].ToString();
                    }
                }
            }
            catch (Exception e)
        ...
        public void get_trans_header()
        {
            ...
            {
                ckon.sqlCon().Open();
                command = "SELECT * FROM [transaction] WHERE TRANSACTION_ID = '" + trans_id + "'";
                CRUD sql = new CRUD();
                ckon.sqlDataRd = sql.ExecuteDataReader(command, ckon.sqlCon());
                ...
                if (ckon.sqlDataRd.HasRows)
                {
                    while (ckon.sqlDataRd.Read())
                    {
                        ...
                        total_trans_header = Convert.ToInt32(ckon.sqlDataRd["TOTAL"].ToString());
                        pay_type = ckon.sqlDataRd["PAYMENT_TYPE"].ToString();
                        cash = Convert.ToInt32(ckon.sqlDataRd["CASH"].ToString());
                        changee = Convert.ToInt32(ckon.sqlDataRd["CHANGEE"].ToString());
                        bank_id = ckon.sqlDataRd["BANK_NAME"].ToString();
                        bank_id2 = ckon.sqlDataRd["BANK_NAME2"].ToString();
                        ...
                        paid_total = Convert.ToInt32(ckon.sqlDataRd["TOTAL"].ToString());
                        date = ckon.sqlDataRd["DATE"].ToString();
                        time = ckon.sqlDataRd["TIME"].ToString();
                    }
                }
            }
            catch (Exception e)
        ...
        public void CreateReceipt(object sender, System.Drawing.Printing.PrintPageEventArgs e)
        {
            ...
            graphic.DrawString(" RECEIPT", new Font("Arial", 10), new SolidBrush(Color.Black), 110, startY);
            //e.Graphics.DrawString(" RECEIPT", new Font("Arial", 10), new SolidBrush(Color.Black), 150, sf);

            graphic.DrawString(nm_store.PadLeft(jrk_nama_store), new Font("Arial", 10), new SolidBrush(Color.Black), 40, startY + 20);
            ...
            String judul = "Article ID".PadRight(15) + "Size".PadRight(6) + "Qty".PadRight(6) + "Disc".PadRight(10) + "Price".PadRight(10) + "Sub Total";
            ...
            {
                ckon.sqlCon().Open();
                command = "SELECT transaction_line.ARTICLE_ID, transaction_line.QUANTITY, transaction_line.DISCOUNT, transaction_line.PRICE, transaction_line.SUBTOTAL,transaction_line.DISCOUNT_DESC, article.SIZE " +
                            "FROM article INNER JOIN transaction_line ON article.ARTICLE_ID = transaction_line.ARTICLE_ID WHERE transaction_line.TRANSACTION_ID = '" + trans_id + "'";
                CRUD sql = new CRUD();
                ckon.sqlDataRd = sql.ExecuteDataReader(command, ckon.sqlCon());
                ...
                if (ckon.sqlDataRd.HasRows)
                {
                    while (ckon.sqlDataRd.Read())
                    {
                        ...
                        disc_desc_real = ckon.sqlDataRd["DISCOUNT_DESC"].ToString();

                        discount_desc = disc_desc_real.Length > 4 ? disc_desc_real.Substring(0, 4) : disc_desc_real;
            ...
            String total_qty = "Sub Total".PadRight(28) + qty2.ToString().PadRight(30) + mata_uang.PadRight(3) + SubTotal_format2;
            ...
            if (voucher == 0)
            {
                graphic.DrawString("Voucher".PadRight(77) + "0", new Font("Arial", 8), new SolidBrush(Color.Black), startX, startY + offset);
                offset = offset + (int)fontHeight + 3;
            }
            else
            {
                String format_voucher = string.Format("{0:#,###}", voucher);
                graphic.DrawString("Voucher".PadRight(77) + mata_uang.PadRight(7) + format_voucher, new Font("Arial", 8), new SolidBrush(Color.Black), startX, startY + offset);
                offset = offset + (int)fontHeight + 3;
            }

            String format_total_header = string.Format("{0:#,###}", total_trans_header);
            graphic.DrawString("Total".PadRight(60) + mata_uang.PadRight(7) + format_total_header, new Font("Arial", 8), new SolidBrush(Color.Black), startX, startY + offset);
            ...
            if (pay_type == "0")
            {
                ...
                graphic.DrawString("Paid".PadRight(60) + mata_uang.PadRight(7) + paid, new Font("Arial", 8), new SolidBrush(Color.Black), startX, startY + offset);
                offset = offset + (int)fontHeight + 3;

                //BUAT PERCABANGAN UNTUK MENGATUR MARGIN UNTUK KEMBALIAN, JIKA KEMBALIAN O ATAU TIDAK O
                if (changee == 0)
                {
                    graphic.DrawString("Change".PadRight(77) + "0", new Font("Arial", 8), new SolidBrush(Color.Black), startX, startY + offset);
                    offset = offset + (int)fontHeight + 3;
                }
                else
                {
                    String change = string.Format("{0:#,###}", changee);
                    graphic.DrawString("Change".PadRight(77) + mata_uang.PadRight(7) + change, new Font("Arial", 8), new SolidBrush(Color.Black), startX, startY + offset);
                    offset = offset + (int)fontHeight + 3;
                }
            ...
            else if (pay_type == "1")
            {
                ...
                {
                    ckon.sqlCon().Open();
                    command = "SELECT * FROM bank WHERE BANK_ID = '" + bank_id + "'";
                    CRUD sql = new CRUD();
                    ckon.sqlDataRd = sql.ExecuteDataReader(command, ckon.sqlCon());
                ...
                graphic.DrawString("Paid".PadRight(60) + mata_uang.PadRight(7) + paid_edc, new Font("Arial", 8), new SolidBrush(Color.Black), startX, startY + offset);
                offset = offset + (int)fontHeight + 3;
            ...
            else if (pay_type == "2")
            {
                ...
                String paid = string.Format("{0:#,###}", cash);
                graphic.DrawString("Cash".PadRight(60) + mata_uang.PadRight(7) + paid, new Font("Arial", 8), new SolidBrush(Color.Black), startX, startY + offset);
                offset = offset + (int)fontHeight + 3;
                ...
                graphic.DrawString("Paid".PadRight(60) + mata_uang.PadRight(7) + total_bayar, new Font("Arial", 8), new SolidBrush(Color.Black), startX, startY + offset);
                offset = offset + (int)fontHeight + 3;
            ...
            else
            {
                ...
                try
                {
                    ckon.sqlCon().Open();
                    command = "SELECT * FROM bank WHERE BANK_ID = '" + bank_id + "'";
                    CRUD sql_bank1 = new CRUD();
                    ckon.sqlDataRd = sql_bank1.ExecuteDataReader(command, ckon.sqlCon());
                    ...
                    command = "SELECT * FROM bank WHERE BANK_ID = '" + bank_id2 + "'";
                    CRUD sql_bank2 = new CRUD();
                    ckon.sqlDataRd = sql_bank2.ExecuteDataReader(command, ckon.sqlCon());
            ...
            graphic.DrawString("Return and Exchanges", new Font("Arial", 8), new SolidBrush(Color.Black), 100, startY + offset);
            offset = offset + (int)fontHeight + 3;

            graphic.DrawString("Within 7 Days With Receipt And Tags Attached", new Font("Arial", 8), new SolidBrush(Color.Black), 40, startY + offset);
            offset = offset + (int)fontHeight + 3;

            graphic.DrawString("Thank You For Coming", new Font("Arial", 8), new SolidBrush(Color.Black), 100, startY + offset);
            offset = offset + (int)fontHeight + 3;

            graphic.DrawString("WWW.3SECOND.CO.ID", new Font("Arial", 8), new SolidBrush(Color.Black), 98, startY + offset);
            offset = offset + (int)fontHeight + 3;
        }
        //end
