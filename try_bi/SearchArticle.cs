    ...
    public partial class SearchArticle : Form
    {
        ...
        private void t_find_article_OnTextChange(object sender, EventArgs e)
        {
            ...
            //{
                ...
                {
                    String sql4 = "select TOP 100 article.ARTICLE_ID, article.ARTICLE_NAME, article.SIZE, article.COLOR, article.PRICE, article._id, inventory.GOOD_QTY FROM article INNER JOIN inventory ON article._id = inventory.ARTICLE_ID WHERE inventory.STATUS = '0' AND inventory.GOOD_QTY >=1  AND article." + combo_ktg2.Text + " = '" + combo_value.Text + "'";
                    get_load_data(sql4);
                    ckon.con.Close();
                }
            //}
        ...
        {
            String sql = "select TOP 100 article.ARTICLE_ID, article.ARTICLE_NAME, article.SIZE_ID, article.COLOR_ID, article.PRICE, article._id, inventory.GOOD_QTY FROM article INNER JOIN inventory ON article._id = inventory.ARTICLE_ID WHERE inventory.STATUS = '0' AND inventory.GOOD_QTY >= 1 AND article." + combo_ktg2.Text + " = '" + combo_value.Text + "'";
            get_load_data(sql);
