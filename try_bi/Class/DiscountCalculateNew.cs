...
using BiensiPOSContext;

namespace try_bi
{
    class DiscountCalculateNew
    {
        private readonly BiensiPOSDataContext _context;

        public DiscountCalculateNew(BiensiPOSDataContext context)
        {
            _context = context;
        }
        ...
        public DiscountMaster Post(Transaction transactionApi)
        {
            ...
            BiensiPOSContext.Store store = _context.Stores.Where(c => c.CODE == transactionApi.storeCode).First();
                ...
                for (int i = 0; i < transactionApi.transactionLines.Count; i++)
                {
                    if (transactionApi.transactionLines[i].price > (transactionApi.transactionLines[i].discount/ transactionApi.transactionLines[i].quantity))
                    {
                        ...
                        if (transactionApi.customerId == "")
                        {
                            ...
                            try
                            {
                                BiensiPOSContext.Article item = _context.Articles.Where(c => c.ARTICLEID == transactionApi.transactionLines[i].article.articleId).First();
                        ...
                        else
                        {
                            ...
                            try
                            {
                                ...
                                if (transactionApi.customerId != "")
                                {
                                    ...
                                    if (custExist)
                                    {
                                        BiensiPOSContext.Customer customer = _context.Customers.Where(c => c.CustId == transactionApi.customerId).First();
                                        ...
                                        for (int l = 0; l < discountRetailsCustGroup.Count; l++)
                                        {
                                            ...
                                            if (startDateTime <= DateTime.UtcNow.ToLocalTime() && endDateTime >= DateTime.Now.ToLocalTime())
                                            {
                                                ...
                                                { 
                                                    ...
                                                    try
                                                    {
                                                        BiensiPOSContext.Itemdimensionbrand item = _context.Itemdimensionbrands.Where(c => c.Description == transactionApi.transactionLines[i].article.brand).First();
                                                        List<Discountretailline> discountLineCustGroup = _context.Discountretaillines.Where(c => c.DiscountRetailId == dc.Id
                                                    ...
                                                    {
                                                        //by item
                                                        BiensiPOSContext.Itemdimensiondepartment item = _context.Itemdimensiondepartments.Where(c => c.Description == transactionApi.transactionLines[i].article.department).First();
                                                        List<Discountretailline> discountLineCustGroup = _context.Discountretaillines.Where(c => c.DiscountRetailId == dc.Id
                                                    ...
                                                    {
                                                        //by item
                                                        BiensiPOSContext.Itemdimensiondepartmenttype item = _context.Itemdimensiondepartmenttypes.Where(c => c.Description == transactionApi.transactionLines[i].article.departmentType).First();
                                                        List<Discountretailline> discountLineCustGroup = _context.Discountretaillines.Where(c => c.DiscountRetailId == dc.Id
                                                    ...
                                                    {
                                                        //by item
                                                        BiensiPOSContext.Itemdimensiongender item = _context.Itemdimensiongenders.Where(c => c.Description == transactionApi.transactionLines[i].article.gender).First();
                                                        List<Discountretailline> discountLineCustGroup = _context.Discountretaillines.Where(c => c.DiscountRetailId == dc.Id
                                                    ...
                                                    {
                                                        //by item
                                                        BiensiPOSContext.Itemdimensionsize item  = _context.Itemdimensionsizes.Where(c => c.Description == transactionApi.transactionLines[i].article.size).First();
                                                        List<Discountretailline> discountLineCustGroup = _context.Discountretaillines.Where(c => c.DiscountRetailId == dc.Id
                                                    ...
                                                    try
                                                    {
                                                        ...
                                                        BiensiPOSContext.Itemdimensioncolor item = _context.Itemdimensioncolors.Where(c => c.Description == transactionApi.transactionLines[i].article.color).First();
                                                        List<Discountretailline> discountLineCustGroup = _context.Discountretaillines.Where(c => c.DiscountRetailId == dc.Id
